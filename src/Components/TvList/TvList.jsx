...
const TvList = () => {
    const {tv,openModal, setOpenModal ,list,setList} = useContext(MovieContext);
  ...
  {tv.map((item) => (
    <div className="mx-auto flex flex-col justify-between h-full p-10 shadow-xl rounded-xl hover:scale-105 ease-in duration-300 bg-[#F9E4C8]" key={item.id}  onClick={()=>setList(item)}>
        <div className="my-0 flex flex-col justify-center">
          ...
          {item.name ?  <h2 className="text-[#27496D] font-sans text-xl font-bold uppercase">{item.name}</h2> : <p className="text-[#27496D] font-sans text-xl font-bold uppercase">Sorry there is no Title</p>}
          <br></br>
        ...
        <div className="flex items-center justify-end  ">
          <button className="border-2 border-[#F9CF93] rounded-md px-4 py-2 bg-[#F9CF93] hover:scale-105  ease-in duration-300 text-[#27496D] font-sans font-bold text-lg"  onClick={()=>setOpenModal(true)}>Detail</button>
