...
const TvList = () => {
    ...
    return (
      <div className="bg-[#FAEEE0]">
          <h2 className="text-[#165f16] font-bold text-xl py-4 border-b-4 w-max border-b-[#fff]">Recommended Tv Series</h2>
    ...
    <div className="mx-auto flex flex-col justify-between h-full p-10 shadow-xl rounded-xl hover:scale-105 ease-in duration-300 bg-[#F9E4C8]" key={item.id}>
        <div className="my-0 ">
          {item.poster_path ? <img src={`https://image.tmdb.org/t/p/w300/${item.poster_path}`}  alt=""/>: null}
            ...
            <p className="text-[#008000] font-sans text-lg italic"><span className="text-[#008000] font-sans text-lg font-bold">Average:</span> {item.vote_average}</p>
