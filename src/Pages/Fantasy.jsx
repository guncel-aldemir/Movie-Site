...
const Fantasy = () => {
    ...
    const categoryDatas = movie.filter((item) => item.genre_ids.includes(14));
   
    const [fantasy,setFantasy] = useState(categoryDatas);
    
  return <div className="bg-[#A2D5F2]">
  <Navbar/>
  <HeaderCategory/>
  ...
  <h2 className=" font-bold text-3xl py-4 px-8 w-max text-[#E50914] font-georgia  ">
        ...
        <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-8 gap-8 bg-[#A2D5F2]">
          {fantasy
            .map((item) => (
              <div
                className="mx-auto flex flex-col justify-between h-full p-10 shadow-xl rounded-xl hover:scale-105 ease-in duration-300 bg-[#FF7E67]"
                key={item.id}
              >
