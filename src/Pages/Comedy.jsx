...
const Comedy = () => {
    ...
    const categoryDatas = movie.filter((item) => item.genre_ids.includes(35));
    
    const [comedy,setComedy] = useState(categoryDatas);
    
  return <div className="bg-[#FF9A3C]">
  <Navbar/>
  <HeaderCategory/>
  ...
  <h2 className=" font-bold text-3xl py-4 px-8 w-max text-[#E50914] font-georgia  ">
        ...
        <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-8 gap-8 bg-[#FF9A3C]">
          {comedy
            .map((item) => (
              <div
                className="mx-auto flex flex-col justify-between h-full p-10 shadow-xl rounded-xl hover:scale-105 ease-in duration-300 bg-[#FFC93C]"
                key={item.id}
              >
